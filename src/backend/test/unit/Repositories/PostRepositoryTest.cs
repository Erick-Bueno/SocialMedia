...
public class PostRepositoryTest
{
    ...
    [Fact]
    public void should_to_list_next_posts_that_user_likes()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>().UseInMemoryDatabase(databaseName: "teste").Options;
        ...
        var result = postRepository.listPostsUserLikeSeeMore(userModelTest.id, new DateTime());
    ...
    public void should_to_list_posts_created_by_user()
    {
        ...
        var result = postRepository.listPostsUserCreated(postModel.userId);
        Assert.IsType<List<PostsLinq>>(result);
    ...
    public void should_to_list_next_posts_created_by_user()
    {
        ...
        var result = postRepository.listPostsUserCreatedSeeMore(postModel.userId, new DateTime());
        Assert.IsType<List<PostsLinq>>(result);

    }

    [Fact]
    public void should_to_list_searched_first_posts()
    {
        var options = new DbContextOptionsBuilder<AppDbContext>().UseInMemoryDatabase(databaseName: "teste").Options;
        ...
        var listPostsImages = new List<PostImagesModel> { postImagesModel, postImagesModel2 }.AsQueryable();
        var listLikes = new List<LikesModel> { LikesModel, LikesModel2 }.AsQueryable();
        var userList = new List<UserModel>{userModelTest, userModelTest2}.AsQueryable();
        ...
        dbSetUsers.As<IQueryable<UserModel>>().Setup(su => su.Provider).Returns(userList.Provider);
        dbSetUsers.As<IQueryable<UserModel>>().Setup(su => su.ElementType).Returns(userList.ElementType);
        dbSetUsers.As<IQueryable<UserModel>>().Setup(su => su.Expression).Returns(userList.Expression);
        dbSetUsers.As<IQueryable<UserModel>>().Setup(su => su.GetEnumerator()).Returns(userList.GetEnumerator());
        ...
        var result = postRepository.findFiveFirstPostsSearched("erick", userModelTest.id);
        Assert.IsType<List<PostsLinq>>(result);
    }
    [Fact]
    public void should_to_list_next_searched_posts()
    {
    var options = new DbContextOptionsBuilder<AppDbContext>().UseInMemoryDatabase(databaseName: "teste").Options;
        ...
        var result = postRepository.findPostsSearchedScrolling(postModel.datePost,"erick",userModelTest.id);
        Assert.IsType<List<PostsLinq>>(result);
    }
}